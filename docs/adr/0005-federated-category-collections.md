...
Collection names and the Match generation search list are derived from `AUCTIONET_LEAF_CATEGORIES` via `referenceCollection()`. The catalog pipeline ensures an empty Qdrant collection for every leaf (including never-sold categories for the Auction House) so the fail-hard search rule cannot be tripped by a missing collection. The per-category vectors directory lives inside its category folder (for example `data/auctionet/items/9-ceramics-porcelain/vectors`), and the seed script derives the collection name from that enclosing category folder. Over-fetching 128 candidates per collection before deduplication is required so the global top 32 still has enough item-level results after collapsing multiple images from the same Auctionet Item. The legacy `references` collection is migrated onto `references-9-ceramics-porcelain` (snapshot recover, same point IDs), then deleted.
