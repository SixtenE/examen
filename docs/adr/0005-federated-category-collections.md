...
The legacy `references` collection (currently ceramics/porcelain) must be renamed to an explicit category name. Seeding scripts need a `--collection` (or category registry) argument per run. Match generation reads the allow-list from configuration rather than hard-coding one collection name. Over-fetching 128 candidates per collection before deduplication is required so the global top 32 still has enough item-level results after collapsing multiple images from the same Auctionet Item.
