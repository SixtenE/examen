...
The web service and catalog cron services must be separate Railway services. An optional scrape-only service uses `railway.scrape.toml` (`cronSchedule = */30 * * * *`, `--skip-embed --skip-seed`). The full pipeline uses `railway.catalog.toml` (`cronSchedule = 0 3 * * *` UTC). Bucket keys live under `scrape/...` so they never collide with Query image Keys. Operators configure categories with `CATALOG_CATEGORIES` (default: ceramics/porcelain and paintings). A run that overlaps the next schedule is skipped by Railway, so each run's scrape/embed budget must fit inside its own interval (30 minutes for scrape-only, 24 hours for the full pipeline) or use `--max-pages` / `--max-items` while bootstrapping.
